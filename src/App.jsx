...
const STYLES = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
  ...
  .gate-logo { font-family: var(--font-display); font-size: 40px; font-weight: 700; text-transform: uppercase; letter-spacing: .04em; color: var(--green); margin-bottom: 6px; }
  .gate-logo span { color: var(--text2); }
  .gate-sub { font-size: 13px; color: var(--text2); margin-bottom: 32px; text-align: center; }
  .gate-label { font-size: 11px; color: var(--text3); text-transform: uppercase; letter-spacing: .06em; margin-bottom: 8px; align-self: flex-start; width: 100%; }
  .gate-input {
    width: 100%;
    background: var(--bg3);
  ...
  .gate-error { font-size: 13px; color: var(--red); text-align: center; }
  .gate-hint { font-size: 11px; color: var(--text3); text-align: center; margin-top: 24px; line-height: 1.6; }
  ...
  .sheet-overlay { position: fixed; inset: 0; background: rgba(0,0,0,.6); z-index: 200; animation: fadeIn .2s ease; }
  @keyframes fadeIn { from{opacity:0} to{opacity:1} }
  .sheet { position: fixed; bottom: 0; left: 50%; transform: translateX(-50%); width: 100%; max-width: 600px; background: var(--bg2); border-radius: 20px 20px 0 0; z-index: 201; max-height: 85vh; display: flex; flex-direction: column; animation: slideUp .25s ease; }
  @keyframes slideUp { from{transform:translateX(-50%) translateY(100%)} to{transform:translateX(-50%) translateY(0)} }
  .sheet-handle { width: 36px; height: 4px; background: var(--bg4); border-radius: 2px; margin: 12px auto 0; flex-shrink: 0; }
  .sheet-hdr { padding: 16px 16px 12px; display: flex; align-items: flex-start; justify-content: space-between; flex-shrink: 0; border-bottom: 1px solid var(--border); }
  .sheet-close { background: var(--bg3); border: none; color: var(--text2); width: 28px; height: 28px; border-radius: 14px; font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  .sheet-body { overflow-y: auto; padding: 16px; flex: 1; }

  .player-detail-name { font-family: var(--font-display); font-size: 24px; font-weight: 700; text-transform: uppercase; letter-spacing: .03em; line-height: 1; }
  .player-detail-meta { font-size: 12px; color: var(--text2); margin-top: 4px; display: flex; gap: 8px; align-items: center; }

  .stat-cards { display: grid; grid-template-columns: repeat(3,1fr); gap: 8px; margin-bottom: 16px; }
  .stat-card { background: var(--bg3); border-radius: 10px; padding: 10px 12px; text-align: center; }
  .stat-card-val { font-family: var(--font-display); font-size: 22px; font-weight: 700; color: var(--green); line-height: 1; }
  .stat-card-label { font-size: 10px; color: var(--text2); text-transform: uppercase; letter-spacing: .05em; margin-top: 3px; }

  .week-chart { margin-bottom: 16px; }
  .week-chart-title { font-size: 11px; color: var(--text3); text-transform: uppercase; letter-spacing: .06em; margin-bottom: 8px; }
  ...
  .bar-fill { width: 100%; border-radius: 3px 3px 0 0; background: var(--green); min-height: 2px; }
  .bar-fill.bye { background: var(--bg4); }
  .bar-label { font-size: 9px; color: var(--text3); }
  .bar-pts { font-size: 9px; color: var(--text2); }

  .detail-stat-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; }
  .detail-stat-row { background: var(--bg3); border-radius: 8px; padding: 8px 10px; display: flex; justify-content: space-between; align-items: center; }
  .detail-stat-label { font-size: 11px; color: var(--text2); }
  .detail-stat-val { font-family: var(--font-display); font-size: 15px; font-weight: 600; }

  .proj-box { background: rgba(0,217,126,.06); border: 1px solid rgba(0,217,126,.2); border-radius: 10px; padding: 12px 14px; margin-bottom: 16px; }
  .proj-box-title { font-size: 10px; color: var(--green); text-transform: uppercase; letter-spacing: .06em; margin-bottom: 8px; font-weight: 600; }
  .proj-pts { font-family: var(--font-display); font-size: 32px; font-weight: 700; color: var(--green); line-height: 1; margin-bottom: 6px; }
  .proj-stats { font-size: 12px; color: var(--text2); }
...
function PasswordGate({ onUnlock }) {
  ...
  function attempt() {
    if (input.trim() === APP_PASSWORD) {
      sessionStorage.setItem("draftiq_auth", "1");
      onUnlock();
    } else {
      setError(true);
      setInput("");
  ...
  return (
    <>
      ...
      <div className="gate-wrap">
        <div className="gate-logo">Draft<span>IQ</span></div>
        <div className="gate-sub">AI-powered Sleeper fantasy football analyst</div>
        <div className="gate-label">Beta access password</div>
        <input
          ...
          autoComplete="off"
        />
        <button className="gate-btn" onClick={attempt} disabled={!input.trim()}>
          Enter
        </button>
        {error && <div className="gate-error">Incorrect password - try again</div>}
        <div className="gate-hint">This is a private beta. Contact the app owner for access.</div>
      </div>
    </>
  );
}

function PlayerSheet({ playerId, players, playerStats, playerProjections, currentSeason, currentWeek, onClose }) {
  ...
  React.useEffect(() => {
    ...
    for (let w = 1; w <= maxWeek; w++) {
      promises.push(
        fetch(`${SLEEPER_BASE}/stats/nfl/regular/${currentSeason}/${w}?season_type=regular`)
          .then(r => r.ok ? r.json() : {})
          .then(data => ({ week: w, stats: data[playerId] || null }))
          .catch(() => ({ week: w, stats: null }))
      );
    }
    Promise.all(promises).then(results => {
      const map = {};
      results.forEach(({ week, stats }) => { map[week] = stats; });
      setWeeklyStats(map);
      setLoading(false);
    });
  }, [playerId, currentSeason, currentWeek]);
  ...
  const pr = playerProjections[playerId] || {};

  const weeks = Object.keys(weeklyStats).map(Number).sort((a, b) => a - b);
  const recentWeeks = weeks.slice(-8);
  const pts = recentWeeks.map(w => weeklyStats[w]?.pts_ppr || 0);
  const maxPts = Math.max(...pts, 1);
  const totalPts = weeks.reduce((sum, w) => sum + (weeklyStats[w]?.pts_ppr || 0), 0);
  const gamesPlayed = weeks.filter(w => (weeklyStats[w]?.pts_ppr || 0) > 0).length;
  const avgPts = gamesPlayed > 0 ? totalPts / gamesPlayed : 0;

  const projStatParts = [];
  if (pr.pass_yd) projStatParts.push(`${Math.round(pr.pass_yd)} pass yds`);
  if (pr.pass_td) projStatParts.push(`${Number(pr.pass_td).toFixed(1)} pass TD`);
  if (pr.rush_yd) projStatParts.push(`${Math.round(pr.rush_yd)} rush yds`);
  if (pr.rush_td) projStatParts.push(`${Number(pr.rush_td).toFixed(1)} rush TD`);
  if (pr.rec) projStatParts.push(`${Number(pr.rec).toFixed(1)} rec`);
  if (pr.rec_yd) projStatParts.push(`${Math.round(pr.rec_yd)} rec yds`);
  if (pr.rec_td) projStatParts.push(`${Number(pr.rec_td).toFixed(1)} rec TD`);

  const totPass = weeks.reduce((s, w) => s + (weeklyStats[w]?.pass_yd || 0), 0);
  const totPassTd = weeks.reduce((s, w) => s + (weeklyStats[w]?.pass_td || 0), 0);
  const totRush = weeks.reduce((s, w) => s + (weeklyStats[w]?.rush_yd || 0), 0);
  const totRushTd = weeks.reduce((s, w) => s + (weeklyStats[w]?.rush_td || 0), 0);
  const totRec = weeks.reduce((s, w) => s + (weeklyStats[w]?.rec || 0), 0);
  const totRecYd = weeks.reduce((s, w) => s + (weeklyStats[w]?.rec_yd || 0), 0);
  const totRecTd = weeks.reduce((s, w) => s + (weeklyStats[w]?.rec_td || 0), 0);
  const seasonTotals = [];
  if (totPass > 0) seasonTotals.push(["Pass Yds", Math.round(totPass)], ["Pass TD", totPassTd]);
  if (totRush > 0) seasonTotals.push(["Rush Yds", Math.round(totRush)], ["Rush TD", totRushTd]);
  if (totRec > 0) seasonTotals.push(["Receptions", totRec], ["Rec Yds", Math.round(totRecYd)], ["Rec TD", totRecTd]);
    ...
    <>
      ...
      <div className="sheet">
        ...
        <div className="sheet-hdr">
          <div>
            <div className="player-detail-name">{name}</div>
            <div className="player-detail-meta">
              <span className={`pos-dot pos-${pos}`} style={{ width:22, height:22, fontSize:8 }}>{POS_MAP[pos] || pos}</span>
              <span>{team}</span>
              {inj && <span style={{ color:"var(--red)", fontWeight:600 }}>{inj}</span>}
            </div>
          </div>
        ...
        <div className="sheet-body">
          <div className="stat-cards">
            <div className="stat-card">
              <div className="stat-card-val">{avgPts > 0 ? avgPts.toFixed(1) : "-"}</div>
              <div className="stat-card-label">Avg Pts</div>
            </div>
            <div className="stat-card">
              <div className="stat-card-val">{totalPts > 0 ? totalPts.toFixed(1) : "-"}</div>
              <div className="stat-card-label">Season Pts</div>
            </div>
            <div className="stat-card">
              <div className="stat-card-val">{gamesPlayed > 0 ? gamesPlayed : "-"}</div>
              <div className="stat-card-label">Games</div>
            </div>
          </div>
          {pr.pts_ppr != null && (
            <div className="proj-box">
              <div className="proj-box-title">Week {currentWeek} Projection</div>
              <div className="proj-pts">{Number(pr.pts_ppr).toFixed(1)} pts</div>
              {projStatParts.length > 0 && <div className="proj-stats">{projStatParts.join(" · ")}</div>}
            </div>
          )}
          {loading ? (
            <div className="loading-center" style={{ padding:24 }}><span className="spinner" /><span>Loading weekly stats...</span></div>
          ) : recentWeeks.length > 0 ? (
            <div className="week-chart">
              <div className="week-chart-title">Last {recentWeeks.length} weeks</div>
              <div className="bars">
                {recentWeeks.map((w) => {
                  const p = weeklyStats[w]?.pts_ppr || 0;
                  const pct = maxPts > 0 ? (p / maxPts) * 100 : 0;
                  return (
                    <div key={w} className="bar-wrap">
                      <div className="bar-pts">{p > 0 ? p.toFixed(0) : ""}</div>
                      <div className={`bar-fill ${p === 0 ? "bye" : ""}`} style={{ height:`${Math.max(pct, p === 0 ? 8 : 4)}%` }} />
                      <div className="bar-label">W{w}</div>
                    </div>
                  );
                })}
              </div>
          ...
          {seasonTotals.length > 0 && (
            <>
              <div className="week-chart-title" style={{ marginBottom:8 }}>Season Totals</div>
              <div className="detail-stat-grid">
                {seasonTotals.map(([label, val]) => (
...
export default function App() {
  const [unlocked, setUnlocked] = useState(
    !APP_PASSWORD || sessionStorage.getItem("draftiq_auth") === "1"
  );
  ...
  async function sendAI(msg) {
    ...
    if (selectedLeague) {
      ...
      context += `Active scoring categories (ONLY these are used in this league): ${activeScoring}\n\n`;
      const fmtPlayer = (id) => {
        const p = players[id];
        if (!p) return id;
        const st = playerStats[id] || {};
            ...
            <div className="page">
              ...
              <div className="about-section">
                ...
                {[
                  ...
                  { n:"4", title:"Check out other teams", desc:"In the Leagues tab, tap any team in the standings to see their full roster and stats - useful for identifying trade targets." },
              ...
              <div className="about-section">
                ...
                {[
                  ...
                  { icon:"⇄", text:"Trade suggestions - get specific proposals with player names from both rosters and an explanation of why the other manager would accept." },
                  { icon:"▶", text:"Start/sit advice - find out who on your bench should be starting based on matchups and projections." },
                  { icon:"◎", text:"Waiver targets - discover available players that fit your roster needs and scoring format." },
              ...
              <div className="about-section">
                ...
                {[
                  ...
                  { icon:"📊", text:"Stats load in the background - player points and projections may take a few seconds to appear after selecting a league." },
              ...
              <div className="about-section">
                ...
                <div className="tip-card">
                  ...
                  <div className="tip-text">All roster, league, and player data comes directly from the Sleeper public API and reflects your live league in real time. Stats and projections are sourced from Sleeper's data provider and update throughout the week.</div>
                </div>
              </div>
