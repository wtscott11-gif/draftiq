...
// Reconstruct full pick ownership from scratch
// Every team originally owns their own picks for each relevant season/round
// Then apply traded picks to transfer ownership
// Exclude picks from seasons whose drafts are complete
function reconstructPickOwnership(rosters, tradedPicks, drafts, currentSeasonInt) {
  // Determine which seasons are still active (draft not complete)
  ...
  // Determine rounds per season from draft settings (fallback to 5)
  const roundsPerSeason = {};
  drafts.forEach(d => {
    if (d.season && d.settings?.rounds) {
      roundsPerSeason[d.season] = d.settings.rounds;
    }
  });

  // Build relevant seasons: current + next 2 years, excluding completed
  const seasons = [];
  ...
  // Also include any future seasons that appear in traded picks
  tradedPicks.forEach(p => {
    const s = String(p.season);
    if (parseInt(s) > currentSeasonInt + 2 && !completedSeasons.has(s)) {
      if (!seasons.includes(s)) seasons.push(s);
    }
  });

  // Build initial ownership map: key = `${season}_${round}_${originalRosterId}` -> currentOwnerId
  ...
  // Apply trades — each traded pick entry shows where a pick ended up
  // roster_id = original team, owner_id = current owner
  tradedPicks.forEach(pick => {
    ...
    if (ownership.has(key)) {
      ownership.set(key, pick.owner_id);
    }
  });

  // Convert map to array of pick objects grouped by current owner
  const result = [];
  ownership.forEach((currentOwnerId, key) => {
    ...
    result.push({
      ...
      roster_id: parseInt(origRosterId),
...
function TradeEvaluator({ myRoster, myRosterId, leagueRosters, leagueUsers, players, playerProjections, multiWeekProjections, currentWeek, userData, reconstructedPicks, onAnalyze, onClose }) {
  ...
  function handleAnalyze() {
    const giveDesc = [
      ...
      ...givePicks.map(key => {
        const pick = myPicks.find(p => pickKey(p) === key);
        return pick ? pickLabel(pick) : key;
      })
    ].join(", ");
    const getDesc = [
      ...getPlayers.map(playerLine),
      ...getPicks.map(key => {
        const pick = opponentPicks.find(p => pickKey(p) === key);
        return pick ? pickLabel(pick) : key;
      })
    ].join(", ");
    const otherTeam = selectedUser.display_name || "the other team";
    const msg = `Evaluate this specific trade: I am giving ${giveDesc} TO ${otherTeam} in exchange for ${getDesc}. Give a full breakdown - value exchange using projection data and pick value, roster impact for both teams, who wins this trade and why, and a clear ACCEPT or DECLINE verdict.`;
    onAnalyze(msg);
    onClose();
  }
...
export default function App() {
  ...
  async function selectLeague(league) {
    ...
    // Fetch traded picks and drafts concurrently to reconstruct ownership
    ...
    if (currentSeason && currentWeek) {
      ...
      try {
        ...
        // Fetch all rounds including 0 (offseason) for dynasty leagues
  ...
  function formatTradesForContext(trades) {
    return trades.map((t, i) => {
      ...
      const pickLines = (t.draft_picks || []).map(pick => {
        ...
        return `${pick.season} ${ordinal(pick.round)} Round Pick (orig: ${fromUser?.display_name || "Team"}) -> ${toUser?.display_name || "Team"}`;
      });
      const allLines = [...playerLines, ...pickLines];
      return `Trade ${i+1} (${date}): ${allLines.length > 0 ? allLines.join(" | ") : "no details available"}`;
    }).join("\n");
  }
  ...
  async function sendAI(msg) {
    ...
    const isTradeReview = /review|completed|past|history|analyze.*trade|trade.*analyz/i.test(msg);
    const windowMs = timeframeToMs(msg);
    ...
    if (selectedLeague) {
      ...
      context += `Active scoring categories: ${activeScoring}\n\n`;
      ...
      if (myRoster) {
        ...
        const picksStr = myPicks.length > 0
          ? `\nOwned picks: ${myPicks.map(p => `${p.season} ${ordinal(p.round)} Round`).join(", ")}`
          : "\nOwned picks: none";
      ...
      const others = leagueRosters
        ...
        .map(r => {
          ...
          const theirPicks = reconstructedPicks
            ...
            .sort((a, b) => a.season !== b.season ? a.season.localeCompare(b.season) : a.round - b.round);
          const picksStr = theirPicks.length > 0
            ? `\n  Owned picks: ${theirPicks.map(p => `${p.season} ${ordinal(p.round)} Round`).join(", ")}`
            : "\n  Owned picks: none";
      ...
      if (filteredTrades.length > 0) {
        context += `COMPLETED TRADES (${filteredTrades.length} found):\n${formatTradesForContext(filteredTrades)}`;
      } else if (isTradeReview) {
        if (allTrades.length > 0) {
          context += `COMPLETED TRADES (all ${allTrades.length} - user requested timeframe filter):\n${formatTradesForContext(allTrades.slice(0, 20))}`;
        } else {
          context += `COMPLETED TRADES: None found for this league.`;
        }
      }
    }
...
IMPORTANT: This may be a dynasty league where trades happen year-round including the offseason. Do not assume trades only happen during the regular season. Use the dates (mm/dd/yy) to determine recency - never use week numbers to infer timing.
...
DRAFT PICKS: Each team's owned draft picks are listed in their roster context with season and round. These have been reconstructed from actual league data accounting for all trades. Treat them as accurate. Value picks by year (nearer = more valuable) and round (lower = more valuable). A 2026 1st round pick is significantly more valuable than a 2028 3rd round pick.

PLAYER VALUATION WEIGHTING:
- HIGHEST WEIGHT: Remaining week-by-week projections (wk_proj fields).
- MEDIUM WEIGHT (close to high): Current season actual stats (wk_actual fields).
- LOW WEIGHT: Previous season history. Use only as baseline for players with small ${currentSeason} samples.

TRADE SUGGESTIONS: Only ask for timeframe clarification when the user sends a brand new open-ended request with NO prior trade conversation context. If the conversation already has context, proceed directly. When clarification IS needed, ask once only:
"I can suggest trades for three timeframes - which would you like?
- Next week
- Next 3 weeks
- Rest of season"

TRADE EVALUATION: When asked to evaluate a specific trade, skip all clarifying questions. Give the full breakdown immediately: value exchange using projection data and pick value, roster impact for both teams, who wins and why, and a clear ACCEPT or DECLINE verdict.

COMPLETED TRADE REVIEW: When the user asks to review completed trades without specifying a timeframe, ask:
"How far back would you like me to look?
- Last day
- Last week
- Last 2 weeks
- Last month
- Full season"
Once they specify, list each trade with its date (mm/dd/yy) and ALL assets - players with positions AND draft picks with year, round, and original team. Ask which specific trade to analyze using the date to identify it.

WAIVER ADVICE: Only suggest players from the AVAILABLE ON WAIVERS list. Never suggest a rostered player.

GENERAL: Maintain full conversation context. Never ask the user to repeat information already provided.
            ...
            <div className="page">
              ...
              <div className="about-section">
                ...
                {[
                  ...
                  { n:"2", title:"Select a league", desc:"All your active leagues for the current season will appear. Tap any league to load its rosters, standings, scoring settings, and draft pick ownership." },
                  { n:"3", title:"Explore your roster", desc:"Tap My Roster to see your full team with last week's actual points, this week's projections, and your owned draft picks with year and round." },
                  { n:"4", title:"Check out other teams", desc:"In the Leagues tab, tap any team in the standings to see their full roster, stats, and owned picks - great for identifying trade targets." },
                  { n:"5", title:"Ask the AI analyst", desc:"Tap AI Analyst and ask anything. The AI has full context of your league including rosters, actual pick ownership, scoring settings, and projections." },
                  { n:"6", title:"Evaluate a trade", desc:"Tap Evaluate a trade to open the Trade Evaluator. Select your opponent, tap players and your actual owned picks on both sides, and tap Analyze for an instant breakdown." },
                ].map(s => (
              ...
              <div className="about-section">
                ...
                {[
                  ...
                  { icon:"⊞", text:"Trade evaluator - select players and your actual owned draft picks on both sides of a deal and get an instant ACCEPT or DECLINE verdict." },
                  { icon:"↺", text:"Completed trade review - ask the AI to review trades from any timeframe including dynasty offseason activity. Full asset detail including picks." },
              ...
              <div className="about-section">
                ...
                {[
                  ...
                  { icon:"⇄", text:"When evaluating a trade, select the opponent first - their roster and picks will not appear until you do." },
                  { icon:"📋", text:"Draft pick ownership is reconstructed from your league's full trade history so it reflects actual current ownership including picks you originally own." },
              ...
              <div className="about-section">
                ...
                <div className="tip-card">
                  ...
                  <div className="tip-text">All roster, league, player, pick ownership, and transaction data comes directly from the Sleeper public API including offseason and dynasty trade activity. Pick ownership is reconstructed from draft settings and full trade history to accurately reflect both original and traded picks. Stats and projections update throughout the week.</div>
                </div>
              </div>
