...
function timeframeToMs(msg) {
  const m = msg.toLowerCase();
  if (m.includes("day") || m.includes("24 hour")) return 1 * 24 * 60 * 60 * 1000;
  if (m.includes("two week") || m.includes("2 week")) return 14 * 24 * 60 * 60 * 1000;
  if (m.includes("week")) return 7 * 24 * 60 * 60 * 1000;
  if (m.includes("month") || m.includes("30")) return 30 * 24 * 60 * 60 * 1000;
  if (m.includes("all") || m.includes("season") || m.includes("everything")) return Infinity;
...
function reconstructPickOwnership(rosters, tradedPicks, drafts, currentSeasonInt) {
  const completedSeasons = new Set();
  drafts.forEach(d => {
    if (d.status === "complete") completedSeasons.add(String(d.season));
  });

  const roundsPerSeason = {};
  drafts.forEach(d => {
    if (d.season && d.settings?.rounds) roundsPerSeason[d.season] = d.settings.rounds;
  });

  const seasons = [];
  for (let y = currentSeasonInt; y <= currentSeasonInt + 2; y++) {
    const s = String(y);
  ...
  tradedPicks.forEach(p => {
    ...
    if (parseInt(s) > currentSeasonInt + 2 && !completedSeasons.has(s) && !seasons.includes(s)) {
      seasons.push(s);
    }
  });
  ...
  seasons.forEach(season => {
    const rounds = roundsPerSeason[season] || 5;
    rosters.forEach(roster => {
      for (let round = 1; round <= rounds; round++) {
        const key = `${season}_${round}_${roster.roster_id}`;
        ownership.set(key, roster.roster_id);
      }
    });
  });

  tradedPicks.forEach(pick => {
    const s = String(pick.season);
    if (!seasons.includes(s)) return;
    const key = `${s}_${pick.round}_${pick.roster_id}`;
    if (ownership.has(key)) ownership.set(key, pick.owner_id);
  });

  const result = [];
  ownership.forEach((currentOwnerId, key) => {
    const [season, round, origRosterId] = key.split("_");
    result.push({ season, round: parseInt(round), roster_id: parseInt(origRosterId), owner_id: currentOwnerId });
  });
...
function TradeEvaluator({ myRoster, myRosterId, leagueRosters, leagueUsers, players, playerProjections, multiWeekProjections, currentWeek, userData, reconstructedPicks, onAnalyze, onClose }) {
  ...
  const myPicks = reconstructedPicks.filter(p => p.owner_id === myRosterId)
    .sort((a, b) => a.season !== b.season ? a.season.localeCompare(b.season) : a.round - b.round);

  const opponentPicks = selectedTeamId
    ? reconstructedPicks.filter(p => p.owner_id === selectedTeamId)
        .sort((a, b) => a.season !== b.season ? a.season.localeCompare(b.season) : a.round - b.round)
    : [];
  ...
  function toggleGivePick(key) { setGivePicks(prev => prev.includes(key) ? prev.filter(x => x !== key) : [...prev, key]); }
  function toggleGetPick(key) { setGetPicks(prev => prev.includes(key) ? prev.filter(x => x !== key) : [...prev, key]); }

  function pickKey(pick) { return `${pick.season}_${pick.round}_${pick.roster_id}`; }

  function pickLabel(pick) {
    const origRoster = leagueRosters.find(r => r.roster_id === pick.roster_id);
    const origUser = origRoster ? leagueUsers.find(u => u.user_id === origRoster.owner_id) : null;
    const isOrig = pick.roster_id === pick.owner_id;
    const suffix = isOrig ? "" : ` (via ${origUser?.display_name || "other"})`;
    return `${pick.season} ${ordinal(pick.round)} Round${suffix}`;
  }
  ...
  function handleAnalyze() {
    const giveDesc = [
      ...givePlayers.map(playerLine),
      ...givePicks.map(key => { const pick = myPicks.find(p => pickKey(p) === key); return pick ? pickLabel(pick) : key; })
    ].join(", ");
    const getDesc = [
      ...getPlayers.map(playerLine),
      ...getPicks.map(key => { const pick = opponentPicks.find(p => pickKey(p) === key); return pick ? pickLabel(pick) : key; })
    ].join(", ");
    const otherTeam = selectedUser.display_name || "the other team";
    const msg = `Evaluate this specific trade: I am giving ${giveDesc} TO ${otherTeam} in exchange for ${getDesc}. Give verdict first then brief reasoning.`;
    onAnalyze(msg);
    onClose();
  }
        ...
        <div className="te-body">
          ...
          <div className="te-side te-side-give">
            ...
            {myPicks.length > 0 && (
              <div className="te-picks">
                <div className="te-section-title" style={{ width:"100%", marginTop:8 }}>Your draft picks</div>
                {myPicks.map(pick => {
                  const key = pickKey(pick);
                  return (
                    <div key={key} className={`te-pick ${givePicks.includes(key) ? "sel-give" : ""}`} onClick={() => toggleGivePick(key)}>
                      {pickLabel(pick)}
                    </div>
                  );
                })}
            ...
            {myPicks.length === 0 && (
              <div style={{ fontSize:11, color:"var(--text3)", marginTop:8 }}>No available draft picks found</div>
            )}
          </div>
            ...
            {!selectedTeamId ? (
              ...
              <>
                ...
                {opponentPicks.length > 0 && (
                  <div className="te-picks">
                    <div className="te-section-title" style={{ width:"100%", marginTop:8 }}>Their draft picks</div>
                    {opponentPicks.map(pick => {
                      const key = pickKey(pick);
                      return (
                        <div key={key} className={`te-pick ${getPicks.includes(key) ? "sel-get" : ""}`} onClick={() => toggleGetPick(key)}>
                          {pickLabel(pick)}
                        </div>
                      );
                    })}
          ...
          {canAnalyze && (
            <div className="te-summary">
              ...
              {[
                ...givePlayers.map(id => { const p=players[id]; return p?`${p.first_name} ${p.last_name}`:id; }),
                ...givePicks.map(key => { const pick=myPicks.find(p=>pickKey(p)===key); return pick?pickLabel(pick):key; })
              ].join(", ")}
              {"\n"}
              <span style={{ color:"var(--green)", fontWeight:600 }}>Receiving: </span>
              {[
                ...getPlayers.map(id => { const p=players[id]; return p?`${p.first_name} ${p.last_name}`:id; }),
                ...getPicks.map(key => { const pick=opponentPicks.find(p=>pickKey(p)===key); return pick?pickLabel(pick):key; })
              ].join(", ")}
            </div>
          )}
...
export default function App() {
  ...
  const [reconstructedPicks, setReconstructedPicks] = useState([]);
  const [statsLoading, setStatsLoading] = useState(false);
  const [selectedPlayerId, setSelectedPlayerId] = useState(null);
  const [showTradeEvaluator, setShowTradeEvaluator] = useState(false);
  const [aiMessages, setAiMessages] = useState([
    { role: "ai", text: "Hi! Select a league then ask me anything - roster weaknesses, trade targets, start/sit decisions, playoff odds. Tap Evaluate a trade to analyze a specific deal." }
  ]);
  ...
  async function selectLeague(league) {
    ...
    const [tradedPicks, drafts] = await Promise.all([
      fetch(`${SLEEPER_BASE}/league/${league.league_id}/traded_picks`)
        .then(r => r.ok ? r.json() : []).catch(() => []),
      fetch(`${SLEEPER_BASE}/league/${league.league_id}/drafts`)
        .then(r => r.ok ? r.json() : []).catch(() => []),
    ]);

    const currentSeasonInt = parseInt(league.season || new Date().getFullYear());
    const picks = reconstructPickOwnership(rostersData, tradedPicks || [], drafts || [], currentSeasonInt);
    setReconstructedPicks(picks);
    ...
    if (currentSeason && currentWeek) {
      ...
      try {
        ...
        const allRounds = [0, ...Array.from({ length: 18 }, (_, i) => i + 1)];
        ...
        tradeResults.flat()
          ...
          .forEach(t => {
            const existing = tradeMap.get(t.transaction_id);
            if (!existing || (t.status_updated || 0) > (existing.status_updated || 0)) {
              tradeMap.set(t.transaction_id, t);
            }
          });
  ...
  function getMyRosterId() {
    return leagueRosters.find(r => r.owner_id === userData?.user_id)?.roster_id;
  }

  function getMyOwnedPicks() {
    const myRosterId = getMyRosterId();
    if (!myRosterId) return [];
    return reconstructedPicks
      .filter(p => p.owner_id === myRosterId)
      .sort((a, b) => a.season !== b.season ? a.season.localeCompare(b.season) : a.round - b.round);
  }

  function getTradesForWindow(ms) {
    if (!ms || ms === Infinity || allTrades.length === 0) return allTrades;
    const cutoff = Date.now() - ms;
    return allTrades.filter(t => (t.status_updated || 0) >= cutoff);
  }

  function formatTradesForContext(trades) {
    return trades.map((t, i) => {
      const date = fmtDate(t.status_updated);
      const playerLines = Object.entries(t.adds || {}).map(([pid, rid]) => {
        const p = players[pid];
        const roster = leagueRosters.find(r => r.roster_id === rid);
        const u = roster ? leagueUsers.find(u => u.user_id === roster.owner_id) : null;
        return `${p ? `${p.first_name} ${p.last_name}(${p.position})` : pid} -> ${u?.display_name || "Team"}`;
      });
      const pickLines = (t.draft_picks || []).map(pick => {
        const toRoster = leagueRosters.find(r => r.roster_id === pick.owner_id);
        const toUser = toRoster ? leagueUsers.find(u => u.user_id === toRoster.owner_id) : null;
        const fromRoster = leagueRosters.find(r => r.roster_id === pick.roster_id);
        const fromUser = fromRoster ? leagueUsers.find(u => u.user_id === fromRoster.owner_id) : null;
        return `${pick.season} ${ordinal(pick.round)} Rd (orig: ${fromUser?.display_name || "?"}) -> ${toUser?.display_name || "?"}`;
      });
      const allLines = [...playerLines, ...pickLines];
      return `Trade ${i+1} (${date}): ${allLines.length > 0 ? allLines.join(" | ") : "no details"}`;
    }).join("\n");
  }

  // Build a flat list of all rostered player IDs for AI context validation
  function getAllRosteredPlayerIds() {
    return new Set(leagueRosters.flatMap(r => r.players || []));
  }
  ...
  async function sendAI(msg) {
    ...
    // Check last 3 messages + current for trade review intent
    const recentText = [
      ...aiMessages.slice(-3).map(m => m.text),
      msg
    ].join(" ");
    const isTradeReview = /review|completed|past|history|analyze.*trade|trade.*analyz|which trade|last day|last week|last month|full season|how far back/i.test(recentText);
    const windowMs = timeframeToMs(recentText);
    const filteredTrades = isTradeReview ? getTradesForWindow(windowMs) : [];
    ...
    if (selectedLeague) {
      const activeScoring = Object.entries(selectedLeague.scoring_settings || {})
        ...
        .map(([k, v]) => `${k}:${v}`)
        .join(", ");
      context += `League: ${selectedLeague.name} (${selectedLeague.settings?.num_teams}-team)\n`;
      context += `Season: ${currentSeason}, Week: ${currentWeek}\n`;
      context += `Scoring: ${activeScoring}\n\n`;
      ...
      const fmtPlayer = (id) => {
        ...
        const wk1 = mw[currentWeek+1];
        if (wk1) parts.push(`nxt:${Number(wk1).toFixed(1)}`);
      ...
      // Full detail for my roster
      const myRoster = getMyRoster();
      if (myRoster) {
        const myPicks = getMyOwnedPicks();
        const picksStr = myPicks.length > 0
          ? `\nPicks: ${myPicks.map(p => `${p.season} ${ordinal(p.round)}`).join(", ")}`
          : "\nPicks: none";
        context += `MY ROSTER:\n${(myRoster.players||[]).map(fmtPlayer).join("\n")}${picksStr}\n\n`;
      }

      // Lighter summary for opponents - name, record, top 8 players, picks
      const others = leagueRosters
        ...
        .map(r => {
          ...
          const topPlayers = (r.players||[]).slice(0,8).map(id => {
            const p = players[id];
            if (!p) return null;
            const pr = playerProjections[id] || {};
            return `${p.first_name} ${p.last_name}(${p.position})${pr.pts_ppr!=null?"["+Number(pr.pts_ppr).toFixed(1)+"pts]":""}`;
          }).filter(Boolean).join(", ");
          const theirPicks = reconstructedPicks
            .filter(p => p.owner_id === r.roster_id)
            .sort((a, b) => a.season !== b.season ? a.season.localeCompare(b.season) : a.round - b.round)
            .map(p => `${p.season} ${ordinal(p.round)}`).join(", ");
          return `${u.display_name||"Team"}(${rec}): ${topPlayers}${theirPicks ? ` | Picks: ${theirPicks}` : ""}`;
        });
      context += `OTHER TEAMS:\n${others.join("\n")}\n\n`;

      if (waiverPool.length > 0) {
        const waiverLines = waiverPool.slice(0,15).map(p =>
          `${p.name}(${p.pos},${p.team})${p.inj?`[${p.inj}]`:""}[proj:${p.thisWeek.toFixed(1)}]`
        );
        context += `WAIVERS:\n${waiverLines.join("\n")}\n\n`;
      }

      if (filteredTrades.length > 0) {
        context += `COMPLETED TRADES (${filteredTrades.length}):\n${formatTradesForContext(filteredTrades)}`;
      } else if (isTradeReview) {
        if (allTrades.length > 0) {
          context += `COMPLETED TRADES (all ${allTrades.length}):\n${formatTradesForContext(allTrades.slice(0,20))}`;
        } else {
          context += `COMPLETED TRADES: None found.`;
        }
      }
    }
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          system: `You are a concise fantasy football analyst. Users are experienced NFL fans - skip basics, be direct.

RESPONSE LENGTH:
- Standard advice (roster, start/sit, waivers): 3-5 sentences max
- Trade suggestions: one line per trade - player names, one-sentence reason, done
- Trade evaluation: verdict first (ACCEPT/DECLINE), then 2-3 sentences why
- Completed trade review: 3-4 sentences on who won and why
- Only expand if user explicitly asks for more detail

TRADE SUGGESTIONS FLOW:
When user asks for trade suggestions and timeframe is known but target type is not, ask ONE question only:
"What are you looking to do?
- Upgrade a starter (QB/RB/WR/TE - which tier: high-end starter, mid-tier, depth)
- Add draft capital (1st/2nd round only carry real value in dynasty)
...
- ONLY suggest players who appear in the league roster data provided. Never suggest free agents or players not on any roster.
...
- Pick value tiers: 1st round = high value anchor piece. 2nd round = meaningful sweetener. 3rd round and later = low value, rarely moves a trade on its own.
- Never suggest a 3rd, 4th, or 5th round pick as a meaningful part of a package for a quality starter.

DYNASTY CONTEXT: This may be a dynasty league. Trades happen year-round. Use dates not week numbers. All rostered players and picks are listed in the context below.

COMPLETED TRADE REVIEW:
When user asks to review completed trades without a timeframe, ask:
"How far back?
- Last day / Last week / Last 2 weeks / Last month / Full season"
Once specified, list trades with dates (mm/dd/yy) and all assets including picks. Ask which to analyze. Use dates to identify trades, not numbers.

WAIVER ADVICE: Only suggest players from the WAIVERS list.

GENERAL: Never repeat clarifying questions already answered. Maintain full conversation context.
...
Season: ${currentSeason}, Week: ${currentWeek}`,
  ...
  const myRoster = getMyRoster();
  const myRosterId = getMyRosterId();
  const myOwnedPicks = getMyOwnedPicks();
      ...
      <div className="app">
            ...
            <div className="page">
              {!selectedLeague ? (
                ...
                <>
                  ...
                  {(() => {
                    const rosterPicks = reconstructedPicks
                      .filter(p => p.owner_id === selectedRoster.roster_id)
                      .sort((a, b) => a.season !== b.season ? a.season.localeCompare(b.season) : a.round - b.round);
                    if (rosterPicks.length === 0) return null;
                    return (
                      <>
                        <div className="pos-label">Owned Draft Picks</div>
                        {rosterPicks.map(pick => {
                          const origRoster = leagueRosters.find(r => r.roster_id === pick.roster_id);
                          const origUser = origRoster ? leagueUsers.find(u => u.user_id === origRoster.owner_id) : null;
                          const isOrig = pick.roster_id === pick.owner_id;
                          return (
                            <div key={`${pick.season}_${pick.round}_${pick.roster_id}`} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 12px", background:"var(--bg3)", borderRadius:10, marginBottom:6 }}>
                              <div style={{ width:30, height:30, borderRadius:7, background:"rgba(167,139,250,.2)", color:"var(--purple)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:700, flexShrink:0 }}>PICK</div>
                              <div style={{ flex:1 }}>
                                <div style={{ fontSize:13, fontWeight:600 }}>{pick.season} {ordinal(pick.round)} Round</div>
                                {!isOrig && <div style={{ fontSize:11, color:"var(--text2)" }}>via {origUser?.display_name || "trade"}</div>}
                              </div>
                            </div>
                          );
                        })}
            ...
            <div className="page">
              ...
              {selectedLeague && myRoster && (
                <>
                  ...
                  {myOwnedPicks.length > 0 && (
                    <>
                      <div className="pos-label">Owned Draft Picks</div>
                      {myOwnedPicks.map(pick => {
                        const origRoster = leagueRosters.find(r => r.roster_id === pick.roster_id);
                        const origUser = origRoster ? leagueUsers.find(u => u.user_id === origRoster.owner_id) : null;
                        const isOrig = pick.roster_id === pick.owner_id;
                        return (
          ...
          {view === "about" && (
            <div className="page">
              ...
              <div className="about-section">
                ...
                {[
                  { n:"1", title:"Enter your Sleeper username", desc:"Tap Search, type your Sleeper username exactly as it appears in the app, and tap Go." },
                  { n:"2", title:"Select a league", desc:"All your active leagues appear. Tap any to load rosters, standings, scoring settings, and draft pick ownership." },
                  { n:"3", title:"Explore your roster", desc:"Tap My Roster to see your full team with projections and owned draft picks by year and round." },
                  { n:"4", title:"Check out other teams", desc:"Tap any team in standings to see their roster and picks - useful for finding trade targets." },
                  { n:"5", title:"Ask the AI analyst", desc:"Tap AI Analyst and ask anything. The AI knows your full roster, pick ownership, scoring settings, and league context." },
                  { n:"6", title:"Evaluate a trade", desc:"Tap Evaluate a trade, select your opponent, pick players and actual draft picks on both sides, and tap Analyze." },
                ].map(s => (
              ...
              <div className="about-section">
                ...
                {[
                  { icon:"◈", text:"Roster analysis - identify weaknesses vs the rest of the league in your scoring format." },
                  { icon:"⇄", text:"Trade suggestions - targeted proposals by position tier and timeframe. AI asks what you need before suggesting." },
                  { icon:"⊞", text:"Trade evaluator - select players and actual owned picks on both sides for an instant ACCEPT or DECLINE verdict." },
                  { icon:"↺", text:"Completed trade review - review any league trade with full asset detail including picks." },
                  { icon:"▶", text:"Start/sit advice - who to start based on matchups and projections." },
                  { icon:"◎", text:"Waiver targets - only suggests truly available players in your league." },
                  { icon:"⬆", text:"Playoff odds - your path to the playoffs based on record and schedule." },
                ].map((t,i) => (
              ...
              <div className="about-section">
                ...
                {[
                  { icon:"💡", text:"Select your league before asking the AI - it needs your league loaded for personalized advice." },
                  { icon:"📊", text:"Stats and projections load in the background after selecting a league - give it a few seconds." },
                  { icon:"👆", text:"Tap any player chip to see their full season stats and weekly breakdown." },
                  { icon:"⇄", text:"When evaluating a trade, select the opponent first - their roster and picks won't appear until you do." },
                  { icon:"📋", text:"Pick ownership is reconstructed from your league's full trade history so it reflects actual current ownership." },
                  { icon:"↺", text:"If the AI starts losing context, a warning appears with a Clear chat button to start fresh without losing league data." },
                ].map((t,i) => (
                  <div key={i} className="tip-card">
                    <div className="tip-icon">{t.icon}</div>
                    <div className="tip-text">{t.text}</div>
              ...
              <div className="about-section">
                ...
                <div className="tip-card">
                  ...
                  <div className="tip-text">All data comes directly from the Sleeper public API including offseason and dynasty trade activity. Pick ownership is reconstructed from draft settings and full trade history. Stats and projections update throughout the week.</div>
                </div>
              </div>
              <div className="disclaimer">
                DraftIQ is an independent tool and is not affiliated with or endorsed by Sleeper. AI advice is for informational purposes only - always use your own judgment.
              </div>
            </div>
          )}
        ...
        {showTradeEvaluator && (
          <TradeEvaluator
            ...
            userData={userData}
            reconstructedPicks={reconstructedPicks}
