...
function timeframeToWeeks(msg) {
  const m = msg.toLowerCase();
  if (m.includes("day") || m.includes("24")) return 1;
  if (m.includes("week") && (m.includes("two") || m.includes("2"))) return 2;
  if (m.includes("week")) return 1;
  if (m.includes("month") || m.includes("30")) return 4;
  if (m.includes("all") || m.includes("season") || m.includes("everything")) return 18;
  return 4;
}
...
export default function App() {
  ...
  async function selectLeague(league) {
    ...
    if (currentSeason && currentWeek) {
      ...
      try {
        ...
        // Fetch all available weeks of trades upfront for deduplication
        // We fetch 18 weeks to cover the full season
        const tradeWeeks = Array.from({ length: 18 }, (_, i) => i + 1);
  ...
  function getTradesForWindow(weeksBack) {
    if (!weeksBack || allTrades.length === 0) return allTrades;
  ...
  function formatTradesForContext(trades) {
    return trades.map((t, i) => {
      ...
      return `Trade ${i+1} (${date}): ${adds.join(" | ")}`;
    }).join("\n");
  }
  ...
  async function sendAI(msg) {
    ...
    // Detect if this is a completed trade review request with a timeframe in the message
    const isTradeReview = /review|completed|past|history|analyze.*trade|trade.*analyz/i.test(msg);
    const weeksBack = timeframeToWeeks(msg);
    const filteredTrades = isTradeReview ? getTradesForWindow(weeksBack) : [];
    ...
    if (selectedLeague) {
      ...
      // Only inject trade history when the user is asking about completed trades
      if (filteredTrades.length > 0) {
        context += `COMPLETED TRADES (${filteredTrades.length} found):\n${formatTradesForContext(filteredTrades)}`;
      } else if (isTradeReview && allTrades.length === 0) {
...
TRADE SUGGESTIONS: Only ask for timeframe clarification when the user sends a brand new open-ended request like "suggest trades" or "what trades should I make" with NO prior trade conversation context. If the conversation already has context about positions, players, or previous suggestions, DO NOT ask for timeframe again - use the context already established and proceed directly with suggestions. When timeframe clarification IS needed, ask once only:
"I can suggest trades for three timeframes - which would you like?
- Next week
- Next 3 weeks
- Rest of season"

TRADE EVALUATION: When asked to evaluate a specific trade (via Trade Evaluator or user description), skip all clarifying questions and go straight to the full breakdown: value exchange using projection data, roster impact for both teams, who wins and why, and a clear ACCEPT or DECLINE verdict.

COMPLETED TRADE REVIEW: When the user asks to review completed trades, ask for a timeframe first:
...
- Last month
- Full season"
Once they respond with a timeframe, the relevant trades will be included in context. List each trade with its date (mm/dd/yy format) and the players involved, then ask which specific trade they want analyzed. Use the date to identify each trade clearly.

WAIVER ADVICE: Only suggest players from the AVAILABLE ON WAIVERS list. Never suggest a rostered player.

GENERAL: Maintain full context of the conversation. If the user references something from earlier in the chat, use that context directly without asking them to repeat themselves.
            ...
            <div className="page">
              ...
              <div className="about-section">
                ...
                {[
                  ...
                  { icon:"↺", text:"Completed trade review - ask the AI to review trades from any timeframe. It will list each trade with its date and ask which one to analyze." },
