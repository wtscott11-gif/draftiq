...
export default function App() {
  ...
  const [aiMessages, setAiMessages] = useState([
    { role: "ai", text: "Hi! I am your fantasy football analyst. Select a league and ask me anything about your roster, weaknesses, or trade targets." }
  ]);
  ...
  async function selectLeague(league) {
    ...
    if (currentSeason && currentWeek) {
      ...
      try {
        const statsWeek = Math.max(1, currentWeek - 1);
  ...
  async function sendAiMessage(msg) {
    ...
    try {
      ...
      const text = data.content?.map(b => b.text || "").join("") || "Sorry, I could not generate a response.";
                      ...
                      <>
                        <div className="section-header">
                          ...
                          <div className="flex-center" style={{ gap: 6 }}>
                            ...
                            {statsLoading
                              ? <span style={{ fontSize: 11, color: "var(--text3)" }}>loading stats...</span>
              ...
              {!selectedLeague && (
                <div className="card mb-16" style={{ background: "rgba(255,184,77,0.05)", borderColor: "rgba(255,184,77,0.2)" }}>
                  <div style={{ color: "var(--amber)", fontSize: 14 }}>
                    No league selected. Go to League Detail first to get personalized advice.
                  </div>
                </div>
              )}
