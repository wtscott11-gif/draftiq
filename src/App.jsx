...
function TradeEvaluator({ myRoster, myRosterId, leagueRosters, leagueUsers, players, playerProjections, multiWeekProjections, currentWeek, userData, myOwnedPicks, allLeaguePicks, onAnalyze, onClose }) {
  ...
  // Picks owned by opponent
  const opponentPicks = selectedTeamId
    ? allLeaguePicks.filter(p => p.owner_id === selectedTeamId)
    : [];
  ...
  function pickLabel(pick) {
    return `${pick.season} ${ordinal(pick.round)} Round Pick`;
  }
          ...
          {canAnalyze && (
            <div className="te-summary">
              ...
              {[
                ...
                ...givePicks.map(key => {
                  const pick = myOwnedPicks.find(p => `${p.season}_${p.round}_${p.roster_id}` === key);
              ...
              {[
                ...
                ...getPicks.map(key => {
                  const pick = opponentPicks.find(p => `${p.season}_${p.round}_${p.roster_id}` === key);
                  return pick ? pickLabel(pick) : key;
                })
              ].join(", ")}
            </div>
...
export default function App() {
  ...
  async function selectLeague(league) {
    ...
    // Fetch traded picks so we know who owns what
    fetch(`${SLEEPER_BASE}/league/${league.league_id}/traded_picks`)
      .then(r => r.ok ? r.json() : [])
      .then(picks => setAllLeaguePicks(picks || []))
      .catch(() => {});
    ...
    if (currentSeason && currentWeek) {
      ...
      try {
        ...
        // Fetch all transaction rounds including 0 (offseason) plus all 18 in-season weeks
        // This ensures dynasty leagues with offseason trades are fully covered
        ...
        // Deduplicate by transaction_id keeping most recent status_updated
  ...
  // Picks currently owned by the user — these are picks that were traded TO them
  // Sleeper traded_picks returns all picks that changed hands; owner_id is the current owner
  function getMyOwnedPicks() {
    const myRosterId = getMyRosterId();
    if (!myRosterId) return [];
    return allLeaguePicks.filter(p => p.owner_id === myRosterId);
  }
  ...
  function formatTradesForContext(trades) {
    return trades.map((t, i) => {
      ...
      // Build player movements: player -> receiving team
  ...
  async function sendAI(msg) {
    ...
    if (selectedLeague) {
      const activeScoring = Object.entries(selectedLeague.scoring_settings || {})
        ...
        .map(([k, v]) => `${k}:${v}`)
        .join(", ");
      context += `League: ${selectedLeague.name} (${selectedLeague.settings?.num_teams}-team, ${selectedLeague.metadata?.scoring_type || "standard"})\n`;
      ...
      const others = leagueRosters
        ...
        .map(r => {
          ...
          const theirPicks = allLeaguePicks.filter(p => p.owner_id === r.roster_id);
      ...
      if (filteredTrades.length > 0) {
        context += `COMPLETED TRADES (${filteredTrades.length} found, filtered by requested timeframe):\n${formatTradesForContext(filteredTrades)}`;
      } else if (isTradeReview) {
        // Still include all trades so AI can reason about the timeframe
        if (allTrades.length > 0) {
          context += `COMPLETED TRADES (all ${allTrades.length} found - user requested timeframe filter, most recent first):\n${formatTradesForContext(allTrades.slice(0, 20))}`;
        } else {
...
IMPORTANT LEAGUE CONTEXT: This may be a dynasty league where trades happen year-round including the offseason. Do not assume trades only happen during the regular season. The COMPLETED TRADES section includes all trades fetched across all weeks including week 0 (offseason). Use the dates provided (mm/dd/yy format) to determine recency - do not use week numbers to infer timing.
...
- DRAFT PICKS: Value picks by year and round. Earlier rounds and nearer years are more valuable. Consider team context when evaluating pick value.

TRADE SUGGESTIONS: Only ask for timeframe clarification when the user sends a brand new open-ended request like "suggest trades" with NO prior trade conversation context. If the conversation already has context about positions, players, or previous suggestions, proceed directly with suggestions using established context. When timeframe clarification IS needed, ask once only:
"I can suggest trades for three timeframes - which would you like?
- Next week
- Next 3 weeks
- Rest of season"

TRADE EVALUATION: When asked to evaluate a specific trade (via Trade Evaluator or user description), skip all clarifying questions and go straight to the full breakdown: value exchange using projection data and pick value, roster impact for both teams, who wins and why, and a clear ACCEPT or DECLINE verdict.
...
- Last month
- Full season"
Once they specify, list each trade with its date (mm/dd/yy) and ALL assets involved including players (with positions) and draft picks (with year, round, and original team). Ask which specific trade to analyze. Use dates not numbers to identify trades.

WAIVER ADVICE: Only suggest players from the AVAILABLE ON WAIVERS list. Never suggest a rostered player.

GENERAL: Maintain full context of the conversation. Never ask the user to repeat information already provided earlier in the chat.
                      ...
                      {myOwnedPicks.map(pick => (
                        <div key={`${pick.season}_${pick.round}_${pick.roster_id}`} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 12px", background:"var(--bg3)", borderRadius:10, marginBottom:6 }}>
                          ...
                          <div style={{ flex:1 }}>
                            ...
                            <div style={{ fontSize:11, color:"var(--text2)" }}>Original team pick</div>
                          </div>
                        </div>
                      ))}
                    </>
            ...
            <div className="page">
              ...
              <div className="about-section">
                ...
                {[
                  ...
                  { n:"3", title:"Explore your roster", desc:"Tap My Roster to see your full team with last week's actual points, this week's projections, and your owned draft picks." },
                  ...
                  { n:"5", title:"Ask the AI analyst", desc:"Tap AI Analyst and ask anything. Use the quick prompts or type your own question. The AI has full context of your league including rosters, picks, scoring settings, and projections." },
                  { n:"6", title:"Evaluate a trade", desc:"Tap Evaluate a trade to open the Trade Evaluator. Select your opponent, tap players and actual draft picks on both sides, and tap Analyze for an instant breakdown." },
                ].map(s => (
              ...
              <div className="about-section">
                ...
                {[
                  ...
                  { icon:"↺", text:"Completed trade review - ask the AI to review trades from any timeframe including dynasty offseason activity. Full asset detail including picks included." },
