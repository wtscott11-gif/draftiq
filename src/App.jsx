...
function TradeEvaluator({ myRoster, myRosterId, leagueRosters, leagueUsers, players, playerProjections, currentWeek, userData, reconstructedPicks, onAnalyze, onClose }) {
  ...
  function playerLine(id) {
    const p = players[id];
    if (!p) return id;
    return `${p.first_name} ${p.last_name}(${p.position},${p.team||"FA"})`;
  }
  ...
  function handleAnalyze() {
    ...
    const msg = `Evaluate this specific trade: I am giving ${giveDesc} TO ${otherTeam} in exchange for ${getDesc}. Value split percentage first, then ACCEPT or DECLINE, then brief reasoning.`;
    onAnalyze(msg);
    onClose();
  }
...
- Trade evaluation: value split first, verdict second, 2-3 sentences reasoning
...
[ACCEPT or DECLINE]
[2-3 sentences max: who wins and why, referencing specific players and pick tier value]
Base the % on total projected asset value weighted by pick tier. No disclaimers, no caveats, no clarifying questions.
...
Once specified, list trades with dates (mm/dd/yy) and all assets including picks. Ask which to analyze. Use dates to identify trades.

WAIVER ADVICE: Only suggest players from the WAIVERS list. Never suggest a rostered player.
            ...
            <div className="page">
              ...
              <div className="about-section">
                ...
                {[
                  ...
                  { icon:"⊞", text:"Trade evaluator - select players and actual owned picks on both sides for an instant value split and ACCEPT or DECLINE verdict." },
